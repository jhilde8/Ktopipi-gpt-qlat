...
def get_cexpr_meson_corr_psnk_psrc():
    fn_base = "cache/auto_contract_cexpr/get_cexpr_sigma_psnk_psrc"
    def calc_cexpr():
...
#point source meson contractions
...
@q.timer(is_timer_fork=True)
def auto_contract_meson_vev_psnk_psrc_psel(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-I0D5-vev-sigma/traj-{traj}/meson_vev_psnk_psrc_psel.lat"
    if get_load_path(fn) is not None:
        return
    cexpr = get_cexpr_meson_vev_psnk_psrc()
    expr_names = get_expr_names(cexpr)
    total_site = q.Coordinate(get_param(job_tag, "total_site"))
    t_size = total_site[3]
    get_prop = get_get_prop()
    ...
    def load_data():
        for pidx in q.get_mpi_chunk(list(range(len(xg_psel_arr)))):
            yield pidx
    @q.timer
    #feval runs for each worker node present. it calculates the correlator data for ONE source position and the full set of sink positions. This data is saved with the 
    #source-sink separation as its index. Each worker node does this, which produces a list of val arrays, which is then summed over in sum_function straight up, because 
    #the dimensions are the same, and all we need to do is sum over source positions. 

    #for vev purposes, we save data in shape (t_sep, t_src), meaning that we can do the same thing, just expand these arrays
    def feval(args):
        pidx = args
        xg_src = q.Coordinate(xg_psel_arr[pidx])
        t_src = xg_src[3]
        ...
        for idx in range(len(xg_psel_arr)):
            ...
            val = eval_cexpr(cexpr, positions_dict=pd, get_prop=get_prop)
            p_list = np.array([1, prob_src, prob_snk])

            val /= p_list
                
            values[x_rel_t, t_src] += val
...
@q.timer
def get_cexpr_pipi_sigma():
    fn_base = "cache/auto_contract_cexpr/get_cexpr_pipi_sigma_corr"
    def calc_cexpr():
        diagram_type_dict = dict()
        #diagram_type_dict[((('src_1','src_2'),1), (('src_2','snk_1'),1), (('snk_1','src_1'),1))] = 'Rectangle_1'
        #diagram_type_dict[((('src_1','snk_1'),1), (('snk_1','src_2'),1), (('src_2','src_1'),1))] = 'Rectangle_2'
        #diagram_type_dict[((('snk_1','snk_1'),1), (('src_1','src_2'),1), (('src_2','src_1'),1))] = 'Vacuum'
        ...
        for mode in [0,1,2,3]:
            exprs += [
                        mk_fac(f"pipi_wave_function(src_1,src_2, {mode}, size, pipi_op_dis_4d_sqr_limit)")
                        * mk_sigma('snk_1',True) * mk_pipi_i0('src_1','src_2') + f"wf_src({mode}) * sigma^dag(0) * pipi_i00(-tsep)",
                    ]
        cexpr = contract_simplify_compile(
                *exprs,
                is_isospin_symmetric_limit=True,
...
@q.timer
def get_cexpr_sigma_pipi():
    fn_base = "cache/auto_contract_cexpr/get_cexpr_sigma_pipi_corr"
    def calc_cexpr():
        diagram_type_dict = dict()
        #diagram_type_dict[((('src_1', 'snk_1'),1), (('snk_1','snk_2'),1), (('snk_2','src_1'),1))] = 'Rectangle_1'
        #diagram_type_dict[((('src_1', 'snk_2'),1), (('snk_2','snk_1'),1), (('snk_1','src_1'),1))] = 'Rectangle_2'
        #diagram_type_dict[((('src_1','src_1'),1), (('snk_1','snk_2'),1), (('snk_2','snk_1'),1))] = 'Vacuum'
        ...
        for mode in [0,1,2,3]:
            exprs += [
                        mk_fac(f"pipi_wave_function(snk_1,snk_2, {mode}, size, pipi_op_dis_4d_sqr_limit)")
                        * mk_pipi_i0('snk_1','snk_2',True) * mk_sigma('src_1') + f"wf_src({mode}) * sigma^dag(0) * pipi_i00(-tsep)",
    ...
    def calc_cexpr():
        ...
        for mode_src in [ 0, 1, 2, 3,]:
            exprs += [
                        ...
                        # * mk_pipi_i0('src_1', 'src_2') + f"wf_src({mode_src}) * pipi_i0(-tsep)"),

                        #(mk_fac(f"pipi_wave_function(snk_1,snk_2, {mode_src}, size, pipi_op_dis_4d_sqr_limit)")
                        # * mk_pipi_i0('src_1', 'src_2',True) + f"wf_src({mode_src}) * pipi_i0^dag(0)"),
...
# -----
#sigma - two pion two point functions

@q.timer(is_timer_fork=True)
def auto_contract_pipi_sigma_corr_psnk_psrc(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-I0D5/traj-{traj}/pipi_sigma_corr.lat"
    if get_load_path(fn) is not None:
        return

    cexpr_ps = get_cexpr_pipi_sigma()
    cexpr_sp = get_cexpr_sigma_pipi()
    expr_names_ps = get_expr_names(cexpr_ps)
    expr_names_sp = get_expr_names(cexpr_sp)
    total_site = q.Coordinate(get_param(job_tag, "total_site"))
    t_size = total_site[3]
    get_prop = get_get_prop()
    psel_prob = get_psel_prob()
    ...
    # sigma wall sink, pipi point source
    ...
    def feval(args):
        ...
        pipiop_tsep = [5] #this can hold many values. For now it just holds one.
        for int_pi_tsep in pipiop_tsep:
            t_src_2 = (t_src - int_pi_tsep) % t_size
            t_snk_2 = (t_snk + int_pi_tsep) % t_size
            for pidx_src_2 in pidx_list_list[t_src_2]:
                if pidx_src_2 in [pidx_snk, pidx_src]:
                    continue
                for pidx_snk_2 in pidx_list_list[t_snk_2]:
                    if pidx_snk_2 in [pidx_snk, pidx_src]:
                        continue
                   
                    prob2 = psel_prob_arr[pidx_snk_2]
                    prob3 = psel_prob_arr[pidx_src_2]
                    prob_sp = prob1 * prob2
                    prob_ps = prob1 * prob3
                    pidx_snk_src_2_list.append((pidx_snk_2, pidx_src_2, prob_sp, prob_ps,))

        values = np.zeros(
                (2, #we want to do this same contraction for <sigma * pipi> and <pipi * sigma> 
                 len(expr_names_ps),
                ), 
                dtype=np.complex128,
                )

        for pidx_snk_2, pidx_src_2, prob_sp, prob_ps in pidx_snk_src_2_list:
            xg_snk_2 = q.Coordinate(xg_psel_arr[pidx_snk_2])
            xg_src_2 = q.Coordinate(xg_psel_arr[pidx_src_2])
            t_snk_2 = xg_snk_2[3]
            t_src_2 = xg_src_2[3]
            pipi_op_tsep_snk = (t_snk_2 - t_snk) % t_size
            pipi_op_tsep_src = (t_src - t_src_2) % t_size
            assert pipi_op_tsep_snk == pipiop_tsep[0]
            pd = {
            ...
            val_sp = eval_cexpr(cexpr_sp, positions_dict=pd, get_prop=get_prop)
            val_ps = eval_cexpr(cexpr_ps, positions_dict=pd, get_prop=get_prop)
            #values must an array of shape [len(expr_names)]
            values[0] += val_ps/prob_ps
            values[1] += val_sp/prob_sp
    ...
    def sum_function(val_list):
        ...
        values = np.zeros(
                (len(pipi_corr_t_sep_list), 
                 2, #once again we have two correlators here
                 len(expr_names_ps),
                 ),
                dtype=np.complex128,
                )
    ...
    pipiop_tsep = [5]
    ld = q.mk_lat_data([
        [ "expr_name", len(expr_names_ps), expr_names_ps, ],
        [ "t_sep", len(pipi_corr_t_sep_list), pipi_corr_t_sep_list, ],
        [ "contraction", 2, ["<pipi * sigma>", "<sigma * pipi>"], ]
        ])
    ld.from_numpy(res_sum)
    ld.save(get_save_path(fn))
    q.json_results_append(f"{fname}: ld sig", q.get_data_sig_arr(ld, q.RngState(), 4))
    for i, en in enumerate(expr_names_ps):
            q.json_results_append(f"{fname}: ld '{en}' sig", q.get_data_sig_arr(ld[i], q.RngState(), 4))
...
def run_auto_contraction(
   ...
   # if use_fsel_prop:
    ...
    #auto_contract_1pt_psel(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob) 
    #auto_contract_meson_vev_psnk_psrc_psel(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob)
    ...
    #auto_contract_pipi_vev_psnk_psrc(job_tag, traj, get_get_prop,get_psel_prob, get_fsel_prob)
    #auto_contract_pipi_sigma_corr_psnk_psrc(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob) 
    #
...
### ------
set_param("48I", "traj_list")(list(range(1102, 1493,10))+list(range(1705, 2176,10)) + list(range(1005, 1096, 10)))
