...
'''
This is a python script that has the capabilities to calculate all correlation functions necessary for pion scattering
in the I=0 and I=2 channels. below is the rough table of contents for this file in terms of functions


1. Meson correlation functions
   a. prefactor functions (mom projection, smearing)
   a. meson expression
   b. meson fsel contraction 
   c. meson psel contraction

2. two pion two point functions
   a. prefactor functions (mom projection, vev subtraction)
   b. pipi fsel contraction
   c. pipi psel contraction

3. vacuum expectation values
   a. to be determined

4. ATW three point functions
   a. prefactors
   b. expression
   c. contraction

5. run_auto_contraction

6. run_job_contraction
...
def auto_contract_meson_corr_psnk_psrc(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-48I-pipisc-test/traj-{traj}/meson_corr_psnk_psrc.lat"
...
def auto_contract_meson_corr_psnk_psrc_psel(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-48I-pipisc-test/traj-{traj}/meson_corr_psnk_psrc_psel.lat"
    if get_load_path(fn) is not None:
        return
    cexpr = get_cexpr_meson_corr_psnk_psrc()
    expr_names = get_expr_names(cexpr)
...
@q.timer(is_timer_fork=True)
def auto_contract_meson_corr_psnk_psrc_smeared(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-gt-test/traj-{traj}/meson_corr_psnk_psrc_smeared.lat"
    if get_load_path(fn) is not None:
        return
    ...
    for pidx_src in range(len(xg_psel_arr)):
        ...
        t_src = xg_src[3]
        for idx_snk in range(len(xg_fsel_arr)):
            xg_snk = q.Coordinate(xg_fsel_arr[idx_snk])
            t_snk = xg_snk[3]
            if idx_snk == pidx_src:
                continue
            tsep_idx = (t_snk - t_src) % t_size
            data_list.append((idx_snk,pidx_src,tsep_idx,))
    
    def load_data():
        data_list_chunk = q.get_mpi_chunk(data_list)
        data_list_size = len(data_list_chunk)
        for data_list_idx, (idx_snk, pidx_src, tsep_idx,) in enumerate(data_list_chunk):
            yield data_list_idx, data_list_size, idx_snk, pidx_src, tsep_idx

    @q.timer
    def feval(args):
        data_list_idx, data_list_size, idx_snk, pidx_src, tsep_idx = args
        ...
        for pidx_src_2 in pidx_list_list[t_src_2]:
            ...
            for idx_snk_2 in fidx_list_list[t_snk_2]:
                if idx_snk_2 in [ pidx_src_2, idx_snk, pidx_src, ]:
                    continue
                prob2 = fsel_prob_arr[idx_snk_2] * psel_prob_arr[pidx_src_2]
                prob = prob1 * prob2
                pidx_snk_src_2_list.append((idx_snk_2, pidx_src_2, prob,)) 
        ...
        for idx_snk_2, pidx_src_2, prob in pidx_snk_src_2_list:
            ...
            pd = {
                    ...
                    "x_1" : ("point-snk", xg_snk.to_tuple(),),
                    "y_1" : ("point-snk", xg_snk_2.to_tuple(),),
                    "size" : total_site,
                    }
            val = eval_cexpr(cexpr, positions_dict=pd, get_prop=get_prop)
            values += val/prob
...
def auto_contract_meson_corr_psnk_psrc_psel_smeared(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-gt-test/traj-{traj}/meson_corr_psnk_psrc_psel_smeared.lat"
    if get_load_path(fn) is not None:
        return
    cexpr = get_cexpr_meson_corr_psnk_psrc_smeared()
    expr_names = get_expr_names(cexpr)
    ...
    def feval(args):
        ...
        #q.displayln_info(0, f"DEBUG: feval:src and snk points found")
        values = np.zeros(
            (#len(pipiop_tsep),
             len(expr_names),
             ),
        ...
        for pidx_snk_2, pidx_src_2, prob in pidx_snk_src_2_list:
            ...
            val = eval_cexpr(cexpr, positions_dict=pd, get_prop=get_prop) #maybe add a get_gt call so we can do it in the cython file
            values += val/prob
        return values, tsep_idx, data_list_idx, data_list_size
...
def auto_contract_pipi_corr_psnk_psrc_psel_V(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-48I-pipisc-test/traj-{traj}/pipi_vev.lat"
    if get_load_path(fn) is not None:
        return
...
#this will be used for both fully connected diagrams (R and C). D is inculded in case the reconstruction doesn't work 
@q.timer(is_timer_fork=True)
def auto_contract_pipi_corr_psnk_psrc_DCR(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-48I-pipisc-test/traj-{traj}/pipi_corr_psnk_psrc.lat"
    if get_load_path(fn) is not None:
        return
...
def auto_contract_pipi_corr_psnk_psrc_psel(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-48I-pipisc-test/traj-{traj}/pipi_corr_psnk_psrc_psel.lat"
    if get_load_path(fn) is not None:
        return
    ...
    def feval(args):
        ...
        for pipiop_tsep_idx, pipiop_tsep in enumerate(pipiop_tsep_list):
            ...
            for pidx_src_2 in pidx_list_list[t_src_2]:
                ...
                for pidx_snk_2 in pidx_list_list[t_snk_2]:
                    ...
                    prob2 = psel_prob_arr[pidx_snk_2] * psel_prob_arr[pidx_src_2]
                    prob = prob1 * prob2
                    pidx_snk_src_2_list.append((pidx_snk_2, pidx_src_2, prob, pipiop_tsep_idx)) #we then save a list of the second source and sink locations
        ...
        for pidx_snk_2, pidx_src_2, prob, pipiop_tsep_idx in pidx_snk_src_2_list:
            xg_snk_2 = q.Coordinate(xg_psel_arr[pidx_snk_2])
            xg_src_2 = q.Coordinate(xg_psel_arr[pidx_src_2])
            t_snk_2 = xg_snk_2[3]
            t_src_2 = xg_src_2[3]
...
def get_cexpr_pipi_3ptATW_corr_psrc_psnk():
    ...
    def calc_cexpr():
        ...
        for mode_src in [ 0, 1, 2, 3, ]:
            for mode_snk in [ 0, 1, 2, 3,]:
                exprs += [
                        # <pi0(t_1+t_2) * O_pipi(t_2)_I2 * pi0(0)^dag>
        ...
        for t_sep, t_sep_idx in enumerate(pipi_int_tsep):
            ...
            for idx_int_1 in fidx_list_list[t_int_1]:
                ...
                for pidx_int_2 in pidx_list_list[t_int_2]:
                    if pidx_int_2 in [pidx_snk, pidx_src, idx_int_1]:
                        continue
            ...
            for idx_int_1, pidx_int_2, prob, t_sep_idx in pidx_int_list:
                ...
                pd = {
                        ...
                        "int_1": ("point-snk", xg_int_1.to_tuple(),),
        ...
        for t_sep, t_sep_idx in enumerate(pipi_int_tsep):
            ...
            for pidx_int_1 in pidx_list_list[t_int_1]:
                ...
                for idx_int_2 in fidx_list_list[t_int_2]:
                    if idx_int_2 in [pidx_snk, pidx_src, pidx_int_1]:
                        continue
...
def run_auto_contraction(
    ...
    fname = q.get_fname()
    fn_checkpoint = f"{job_tag}/auto-contract-48I-pipisc-test/traj-{traj}/checkpoint.txt"
...
def run_job_contraction(job_tag, traj):
    ...
    #
    fns_produce = [
            f"{job_tag}/auto-contract-48I-pipisc-test/traj-{traj}/checkpoint.txt",
            #
            ]
    fns_need = [
