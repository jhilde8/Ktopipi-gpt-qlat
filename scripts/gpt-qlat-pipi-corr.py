...
#why is it a cosine and not a full exponential? 
...
def get_cexpr_meson_corr_psnk_psrc():
    ...
    def calc_cexpr():
        ...
        exprs = [
                mk_fac(1) + f"1",
                mk_fac(f"wave_function(x_1,x_2, {0}, size)")
                  * mk_k_0("x_2",True) * mk_k_0("x_1")
                   + f"wf(0) * K_0^dag(0) * K_0(-tsep)",
                ]
        ...
        for mode_src in [ 0, 1, 2, 3, ]:
            for mode_snk in [ 0, 1, 2, 3,]:
                exprs += [
                        ...
                        #+ f"wf_snk({mode_snk}) * wf_src({mode_src}) * pipi_i22+^dag(0) * pipi_i22(-tsep)",
                        #
                        # <pi+(t_1+t_2) * 2pi(t_2) * pi+(0)^dag>
                        #mk_fac(f"wave_function(x_1,x_2,{mode},size)")
                        #* mk_fac(f"pipi_wave_function(src_1,src_2, {mode_src}, size, pipi_op_dis_4d_sqr_limit)")
                        #* mk_pi_p("x_2", True) * mk_pipi_i20("src_1,src_2") * mk_pi_p("x_1")
                        #+ f"wf({mode}) * pi+^dag(0) * pipi_i20(-t_int) * pi+(-t)",
                        # 
...
### ------
set_param("48I", "traj_list")(list(range(1595, 1696, 10)))
set_param("48I", "measurement", "auto_contractor_chunk_size")(128)
set_param("48I", "measurement", "meson_tensor_t_sep")(12)
set_param("48I", "measurement", "ATW3pt_src_snk_t_sep")(24)
set_param("48I", "measurement", "pipi_op_t_sep")(1)
set_param("48I", "measurement", "pipi_op_dis_4d_sqr_limit")(25.0)
set_param("48I", "measurement", "pipi_corr_t_sep_list")(list(range(1, 24)))
set_param("48I", "measurement", "pipi_tensor_t_sep_list")([ 1, ])
