...
def auto_contract_meson_corr_psnk_psrc_psel(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-pos/traj-{traj}/meson_corr_psnk_psrc_psel.lat"
    if get_load_path(fn) is not None:
        return
    cexpr = get_cexpr_meson_corr_psnk_psrc()
    expr_names = get_expr_names(cexpr)
    ...
    def feval(args):
       ...
       # values = np.zeros((total_site[0]//2), (total_site[1]//2)+1, (total_site[2]//2)+1, (total_site[3]//2)+1, len(expr_names),), dtype=np.complex128) #accumulation over all sources in this call
        ...
        for pidx in pidx_list: #iterate through the index list we passed in
            ...
            for idx in range(len(xg_psel_arr)): #use all randomly selected points as sinks
                ...
                #just accumulate the values, the first cexpr counts the number of times a relative coordinate configuration event occurs, so we can divide later.
                values_ind[abs(x_rel[0]),abs(x_rel[1]),abs(x_rel[2]),abs(x_rel_t)] += val
    ...
    res_sum *= 1.0 / (t_size * (total_volume / t_size))
    ld = q.mk_lat_data([
        [ "x_sep", x_size, [ str(q.rel_mod(x,x_size)) for x in range(x_size) ], ],
        [ "y_sep", y_size, [ str(q.rel_mod(y,y_size)) for y in range(y_size) ], ],
        [ "z_sep", z_size, [ str(q.rel_mod(z,z_size)) for z in range(z_size) ], ],
        [ "t_sep", t_size, [ str(q.rel_mod(t,t_size)) for t in range(t_size) ], ], 
        [ "expr_name", len(expr_names), expr_names, ],
        ])
    ld.from_numpy(res_sum)
...
def auto_contract_pipi_corr_psnk_psrc(job_tag, traj, get_get_prop, get_psel_prob, get_fsel_prob):
    fname = q.get_fname()
    fn = f"{job_tag}/auto-contract-test/traj-{traj}/pipi_corr_psnk_psrc.lat"
    if get_load_path(fn) is not None:
        return
    ...
    def load_data():
        lsize = 8 #number of indices we want to pass through for each worker
        snk_list = []
        src_list = []
        tsep_list = []
        ...
        for data_list_idx, (pidx_snk, pidx_src, t_sep_idx,) in enumerate(data_list_chunk):
            snk_list.append(pidx_snk)
            src_list.append(src_list)
            tsep_list.append(t_sep_idx)

            if len(snk_list) == lsize:
                #yield data_list_idx, data_list_size, pidx_snk, pidx_src, t_sep_idx
                yield data_list_idx, data_list_size, snk_list, src_list, tsep_list


    @q.timer
    def feval(args):
        #data_list_idx, data_list_size, pidx_snk, pidx_src, t_sep_idx = args
        data_list_idx, data_list_size, snk_list, src_list, tsep_list = args
        val_acc = np.zeros()

        for l_idx in range(len(snk_list)): 
        
            pidx_snk = snk_list[l_idx]
            pidx_src = src_list[l_idx]
            t_sep_idx = tsep_list[l_idx]
...
### ------
set_param("48I", "traj_list")(list(range(1202, 1302, 10)))
